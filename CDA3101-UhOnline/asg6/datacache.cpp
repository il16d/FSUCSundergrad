 ...
 * Compile: "g++ -std=c++11 -o datacache.exe datacache.cpp" *
 *                                                          *
 ***********************************************************/
#include <iostream>
#include <iomanip>
...
void PrintBodyToFile();
void PrintFooterToFile();
void ExtractDATFile();
...
{
	if (argc != 1) //check for bad parameters
	{
		cerr << "Usage: " << "< trace.dat > (output).stats"<< endl;
		exit(1);
	}
...
void PrintHeaderToFile()
{
	...
	cout << "   " << traceConfig[0] << " " << traceConfig[1] << "-way set associative entries" << endl;
	cout << "   of line size " << traceConfig[2] << " bytes" << endl << endl << endl;
...
void PrintBodyToFile()
{
	...
	for (size_t i = 0; i < traceData.size(); i++)
	{
		...
		if (p.isGood)
		{
			cout << setw(4) << right << dec << i - misaligned + 1; //list starts at 1
			cout << setw(7) << right << accessType;
			cout << setw(9) << right << hex << traceData[i].address;
			cout << setw(8) << right << hex << p.tag;
			cout << setw(6) << right << dec << p.index;
			cout << setw(7) << right << dec << p.offset;
			cout << setw(7) << right << dec << p.result;
			cout << setw(8) << right << dec << p.memrefs;
			cout << endl;
		}
		else
		{
...
void PrintFooterToFile()
{
	...
	cout << "-----------------------------" << endl;
	cout << "Total hits       : " << dec << hit << endl;
	cout << "Total misses     : " << miss << endl;
	cout << "Total accesses   : " << totalAccess << endl;
	cout << fixed << setprecision(6); //set formatting
	if(totalAccess != 0){
		cout << "Hit ratio        : " << hit * 1.0 / totalAccess << endl;
		cout << "Miss ratio       : " << miss * 1.0 / totalAccess << endl << endl;
	}
}
